                    ...
                    @foreach ($ropas as $ropa)
                        <tbody>
                            <tr>
                                <td>{{ ++$i }}</td>
                                <td>{{ $ropa->prenda}}</td>
                                <td>{{ isset($ropa->tipoColor_id) ? $colors->get('descripcion', $colors->find($ropa->tipoColor_id)->descripcion) : ''}}</td>
                                <td>{{ $ropa->stock}}</td>
                                <td>{{ $ropa->precio }}</td>
                                <td>{{ $ropa->talla }}</td>
                                <td>{{ $ropa->estado }}</td>
                                <td>{{ $ropa->imagenRef}}</td>
                                <td>
                                    <form action="{{ route('ropas.destroy', $ropa->id) }}"
                                        method="POST">
